...
{ 
  ...
  #include "roundconstants_randomness.h"
   
  static constexpr unsigned rounds = 19;     // Number of rounds
  static constexpr unsigned numofboxes = 32; // Number of Sboxes
  ...
  LowMC2(const __m128i & key_ = _mm_setzero_si128())
    : //rounds(128),
      //numofboxes(63),
...
template <typename T>
auto LowMC2<__mX>::encrypt_(const std::array<T, blocklen> & message) const   
{
...
template <typename __mX>
template <typename T>
void LowMC2<__mX>::Substitution_(std::array<T, blocklen> & message) const
{
